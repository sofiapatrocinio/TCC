...
\label{chap:Proposta}
Neste capítulo, é retomado o contexto em que o trabalho pretende contribuir, apresentando a proposta do Trabalho de Conclusão de Curso. Inicialmente, na seção \hyperref[sec:Contextualizacao]{5.1 - Contextualização}, é 
apresentado o domínio em que o estudo está inserido. Adicionalmente, com intuito de apresentar adequadamente a aplicação, a seção \hyperref[sec:Detalhamento da Aplicacao]{5.2 - Detalhamento da Aplicação} é exposta. A \hyperref[sec:Prova de Conceito]{5.3 - Prova de Conceito}, 
que detalha como os testes de usabilidade serão  feitos, também é coberta no capítulo. A seção \hyperref[sec:Primeiro Ciclo]{5.4 - Primeiro Ciclo de Teste} esclarece sobre "como'' e "o que'' foi testado, além dos resultados. Com base nos insumos obtidos no primeiro ciclo de teste, 
a seção \hyperref[sec:Melhorias Propostas]{5.5 - Melhorias Propostas} elucida sobre a incorporação das melhorias elicitadas junto ao público alvo no aplicativo em estudo, o que demandou refinamentos no protótipo de alta fidelidade, em especial no fluxo de \hyperref[sec:Onboarding]{5.5.1 - \textit{Onboarding}}, 
e na disposição de informações em geral. A seção \hyperref[sec:Segundo Ciclo]{5.6 - Segundo Ciclo de Teste} também é abordada, procurando mostrar os resultados obtidos com a análise da versão do aplicativo evoluída, ou seja, que teve as melhorias do primeiro ciclo incorporadas. 
Com base nesses resultados, a \hyperref[sec:Matriz SWOT]{5.7 - Matriz SWOT} apresenta as forças, fraquezas, ameaças e oportunidades identificadas durante a análise da versão evoluída do aplicativo. Ademais, encontra-se a \hyperref[sec:Backlog de Melhorias]{5.8 - \textit{Backlog} de Melhorias} 
...
Por fim, tem-se o \hyperref[sec:Resumo Proposta]{5.9 - Resumo do Capítulo}.
