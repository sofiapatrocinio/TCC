...
Em relação à duração média para concluir as tarefas, destaca-se a tarefa F04, que envolve visualizar línguas por família linguística, apresentando uma duração média bastante elevada de 198.4 segundos (ou aproximadamente 3.3 minutos). Isso sugere dificuldades substanciais na execução 
dessa tarefa por parte dos participantes. Da mesma forma, a tarefa F07, que consiste em visualizar imagens relacionadas a palavras em uma língua específica, teve uma duração média de 65.3 segundos.
...
No segundo ciclo de testes, houve melhorias em algumas métricas de usabilidade. As taxas de sucesso direto foram mais consistentes, com todas as tarefas apresentando taxas de sucesso direto acima de 20\%, indicando uma melhoria na usabilidade das funcionalidades do aplicativo. Além disso, 
as taxas de misclick foram geralmente mais baixas em comparação com o primeiro ciclo, sugerindo uma melhoria na clareza das opções disponíveis. A duração média para concluir as tarefas também diminuiu em algumas funcionalidades, o que pode indicar uma melhoria na eficiência da navegação 
