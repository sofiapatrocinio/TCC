...
neste trabalho, incluindo \hyperref[sec:Persona]{Persona} (Seção \hyperref[sec:Persona]{2.1}), \hyperref[sec:Usabilidade]
{Usabilidade} (Seção \hyperref[sec:Usabilidade]{2.2}), \hyperref[sec:Experiencia de Usuario]{Experiência de Usuário} 
(Seção \hyperref[sec:Experiencia de Usuario]{2.3}) e \hyperref[sec:Teste de Comparação]{Teste de Comparação} (Seção \hyperref[sec:Teste de Comparação]{2.4}). 
Serão elucidadas ainda técnicas e métricas relevantes na avaliação da usabilidade e da experiência de usuário. Por fim, tem-se o 
\hyperref[sec:Resumo do Capítulo]{Resumo do Capítulo} (Seção \hyperref[sec:Resumo do Capítulo]{2.5}). 
...
indígenas como área de interesse. Adicionalmente, foram definidas personas no intuito de criar uma 
...
\begin{figure}[h!]
  ...
  \begin{adjustbox}{center}
	  \includegraphics[width=1.2\textwidth]{figuras/persona.eps}
  \end{adjustbox}
...
em: \url{https://www.nngroup.com/articles/definition-user-experience/} (último acesso: Março 2024)} - em português, Experiência de Usuário - foi popularizado por Donald Norman, 
...
No caso do presente trabalho, procurou-se usar testes de comparação para comparar a solução melhorada do aplicativo Multilind, orientando-se pela aplicação das heurísticas de Nielsen, com a solução inicial 
do aplicativo. O foco da comparação será a experiência de usuário, e o método para aferir essa experiência foi o Questionário \textit{AttrakDiff}. Documentou-se os resultados obtidos a partir do questionário, 
procurando destacar: Forças, Fraquezas, Oportunidades e Ameaças. Ao identificar forcas e fragilidades, sob a perspectiva da experiência de usuário, buscou-se reconhecer e evidenciar, respectivamente, oportunidades 
e ameaças. Tal noção quanto ao comportamento da solução proposta - no caso, o aplicativo melhorado - permitiu não apenas responder a Questão de Pesquisa inerente ao trabalho, como também orientar trabalhos futuros 
para evolução do aplicativo Multilind, mantendo-o mais aderente às expectativas do público alvo. Por fim, acredita-se que essa experiência, já documentada, auxilie terceiros na compreensão sobre os principais 
impactos da usabilidade na experiência de usuário com a apresentação de insumos.
...
Em relação à Usabilidade, são detalhadas as cinco sub características inerentes à avaliação, conforme a norma ISO 9126-1 \cite{iso9126}, que incluem inteligibilidade, apreensibilidade, operacionalidade, 
